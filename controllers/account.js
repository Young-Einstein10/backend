...
const signup = (request, response) => {
  const {firstname, lastname, email, password, gender, jobrole, department, address,  role} = request.body; 
...
1      // console.log(results.rows)
    ...
    bcrypt.hash(password, 8).then(
      (hash) => {
        pool.query('INSERT INTO employees (firstname, lastname, email, password, gender, jobrole, department, address, role) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)', [firstname, lastname, email, hash, gender, jobrole, department, address, role], (error, results) => {
            if (error) {
              throw error
            }
            // console.log(results)
          ...
          response.status(201).json({
            ...
            data: {
              ...
              userId: request.body.id,    // NB: userId not showing ???
